...
function WorkerOutput({ lines }: { lines: WorkerOutputLine[] }) {
  return <div className="min-h-[510px] bg-[#202b29] p-4 rounded-lg"><div className="mb-4 flex items-center gap-2 font-mono text-[9px]   text-[#82928c]"><span className="size-1.5 rounded-full bg-[#78b88b]" />Deployment output</div>{lines.length ? <div className="space-y-1.5">{lines.map(({ timestamp, level, message, deployment_id }, index) => <p key={`${timestamp}-${index}`} className="font-mono text-[11px] leading-5 text-[#c6d0cb]"><span className="mr-3 text-[#71817b]">{new Date(timestamp).toLocaleTimeString()}</span><span className={cn("mr-3", level === "error" ? "text-[#e87962]" : level === "warn" ? "text-[#e3a65a]" : "text-[#78b88b]")}>{level.toUpperCase()}</span>{deployment_id && <span className="mr-3 rounded border border-[#40504b] px-1.5 py-0.5 text-[9px] text-[#9fb0aa]">{shortDeploymentID(deployment_id)}</span>}{message}</p>)}</div> : <p className="pt-16 text-center font-mono text-[10px]   text-[#71817b]">No deployment output captured yet</p>}</div>
}
