...
export function parseArgs(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === "--help" || arg === "-h")
            ...
            if (!result.template)
                throw new Error(`${arg} requires a template name`);
        }
            ...
            addDirectory(result, arg);
    }
    return result;
}
function addDirectory(result, directory) {
...
function findTemplate(id) {
    ...
    if (!template)
        throw new Error(`Unknown template ${JSON.stringify(id)} (available: ${templates.map((entry) => entry.id).join(", ")})`);
    return template;
}
function isInteractiveTerminal() {
    return Boolean(process.stdin.isTTY && process.stdout.isTTY);
}
async function promptWithReadline(message) {
...
async function writeTemplate(template, destination, name, now) {
    ...
    if (!destinationInside(source, templatesDirectory))
...
export async function run(args, options = {}) {
    ...
    if (!(await directoryIsEmpty(destination))) {
        ...
        if (!overwrite && interactive) {
            const answer = (await prompt(`Target directory ${directory} is not empty. Remove existing files and continue? [y/N] `))
                .trim()
                .toLowerCase();
            overwrite = answer === "y" || answer === "yes";
        }
        if (!overwrite)
            throw new Error(`Target directory ${destination} is not empty. Use --overwrite to remove its contents.`);
...
export async function readStarterTemplate() {
