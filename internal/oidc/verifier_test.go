...
func TestVerifierBrowserFlowCreatesSession(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/token":
			_ = r.ParseForm()
			_ = json.NewEncoder(w).Encode(map[string]any{
				"access_token": signedJWT(t, privateKey, issuer, "platform", map[string]any{"sub": "user-123", "email": "person@example.com"}),
				"token_type":   "Bearer",
				"expires_in":   1800,
			})
		case "/userinfo":
	...
	verifier := NewBrowserVerifier(issuer, "platform", "email", "client-id", "secret", "https://platform.example.com/internal/auth/callback", server.Client())
