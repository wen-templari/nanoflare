...
app.put("/objects/:key", async (c) => {
  await c.env.OBJECTS.put(c.req.param("key"), c.req.raw.body!);
  return c.body(null, 204);
});
app.get("/objects/:key", async (c) => {
  const object = await c.env.OBJECTS.get(c.req.param("key"));
  return object ? new Response(object.body) : c.notFound();
});
